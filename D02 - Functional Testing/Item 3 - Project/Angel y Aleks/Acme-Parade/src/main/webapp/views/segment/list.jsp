...
<display:table name="segments" id="row" requestURI="${requestURI}"
	...
	<jstl:set var="openPath" value="${row.parade.openPath}" />
...
<jstl:if test="${segments.size() > 0}">
	<security:authorize access="hasRole('BROTHERHOOD')">
		<jstl:if test="${!openPath}">
			<a href="segment/open.do?parade=${param['parade']}"><spring:message
					code="segment.openPath" /></a>
		</jstl:if>
		<jstl:if test="${openPath}">
			<a href="segment/close.do?parade=${param['parade']}"><spring:message
					code="segment.closePath" /></a>
		</jstl:if>
	</security:authorize>
</jstl:if>
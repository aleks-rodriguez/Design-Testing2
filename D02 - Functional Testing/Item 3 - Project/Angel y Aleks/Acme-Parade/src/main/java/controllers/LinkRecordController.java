...
public class LinkRecordController extends AbstractController {
	...
	public ModelAndView create(@RequestParam(defaultValue = "0") final int idLink) {
		...
		//		Collection<Brotherhood> col;
		//		col = this.brotherhoodService.findAllBrotherhood();
		//		col.remove(this.brotherhoodService.findActorByUserAccount(LoginService.getPrincipal().getId()));
		result.addObject("brotherhoodList", this.brotherhoodService.getAllBrotherhoodsWithoutYou(LoginService.getPrincipal().getId()));

...
public class RequestController extends AbstractController {
	...
	public ModelAndView delete(@RequestParam final int idRequest) {
		ModelAndView result;
		result = null;
		Parade p;
		p = this.reqService.findByRequestId(idRequest);
		final Request r;
		r = this.reqService.findOne(idRequest);
		try {
			this.reqService.delete(r.getId());
			result = this.custom(new ModelAndView("redirect:/request/member/list.do?procId=" + p.getId()));
		} catch (final Throwable oops) {
			result = this.createEditModelAndView(this.reqService.findOne(idRequest), "request.commit.error");
		}

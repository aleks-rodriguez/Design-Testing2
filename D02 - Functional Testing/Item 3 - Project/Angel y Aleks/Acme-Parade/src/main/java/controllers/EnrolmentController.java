...
@RequestMapping(value = {
	"/enrolment/member", "/enrolment/brotherhood", "enrolment/administrator", "/enrolment"
})
public class EnrolmentController extends AbstractController {
	...
	public ModelAndView deleteMember(@RequestParam final int idMember) {
		...
		user = LoginService.getPrincipal();
		Brotherhood b;
		b = this.actorService.findByUserAccount(user.getId());
		final Enrolment e;
	...
	public ModelAndView show(@RequestParam final int idMember) {
		...
		final ActorForm form = this.actorService.map(m, null);
	...
	protected ModelAndView createEditModelAndView(final ActorForm member) {
		return this.createEditModelAndView(member, null);
	}

	protected ModelAndView createEditModelAndView(final ActorForm member, final String message) {
		ModelAndView result;
		Map<String, String> map;
		map = new HashMap<String, String>();
		map.put("id", String.valueOf(member.getId()));

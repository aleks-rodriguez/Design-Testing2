...
public class MessageService {
	...
	//Create Message for request change status
	public void createMessageNotifyRequestChangeStatus(final Actor a, final Request r) {
		MessageEntity message;
		message = new MessageEntity();

		message.setSender(a); //cambiar por un admin predeterminado
		message.setBody("One of your request in a procession change its status to " + r.getStatus() + " Go to see it");
		message.setMomentsent(new Date());
	...
	//Create Message for enrol a member
	public void createMessageNotifyEnrolAMember(final Member a, final Brotherhood bro) {
		MessageEntity message;
		message = new MessageEntity();

		message.setSender(bro); //cambiar por un admin predeterminado
		message.setBody("The brotherhood " + bro.getTitle() + " enrol you! Congratulations!");
		message.setMomentsent(new Date());
		ArrayList<String> tags;
		tags = new ArrayList<String>();
		tags.add("Notification");
		message.setTags(tags);
		message.setSubject("Notification Enrol Member");
		message.setPriority("NEUTRAL");
		message.setBox(new ArrayList<Box>());
		final Collection<Actor> actRec = new ArrayList<Actor>();
		actRec.add(a);
	...
	//Create Message for drop out of brotherhood
	public void createMessageNotifyDropOutBrotherhood(final Member a, final Brotherhood bro) {
		MessageEntity message;
		message = new MessageEntity();

		message.setSender(a); //cambiar por un admin predeterminado
		message.setBody("The brotherhood " + bro.getTitle() + " drop out :( ");
		message.setMomentsent(new Date());
		ArrayList<String> tags;
		tags = new ArrayList<String>();
		tags.add("Notification");
		message.setTags(tags);
		message.setSubject("Notification Drop Out Brotherhood");
		message.setPriority("NEUTRAL");
		message.setBox(new ArrayList<Box>());
		final Collection<Actor> actRec = new ArrayList<Actor>();
		actRec.add(a);
	...
	//Create Message for create procession
	public void createMessageNotifyCreateProcession(final Brotherhood a) {
		MessageEntity message;
		message = new MessageEntity();

		message.setSender(a); //cambiar por un admin predeterminado
		message.setBody("New procession has been created");
		message.setMomentsent(new Date());
		ArrayList<String> tags;
		tags = new ArrayList<String>();
		tags.add("Notification");
		message.setTags(tags);
		message.setSubject("Notification Procession Published");
		message.setPriority("NEUTRAL");
		message.setBox(new ArrayList<Box>());
		Collection<Member> colmem;
		colmem = this.actorService.getAllMemberByBrotherhood(a.getId());
		Collection<Actor> colact;
		colact = new ArrayList<>();
		for (final Member member : colmem) {
			final int idUserMember;
			idUserMember = member.getAccount().getId();
			Actor act;
			act = this.actorService.findActorByUserAccount(idUserMember);
			colact.add(act);
		}
		message.setReceiver(colact);

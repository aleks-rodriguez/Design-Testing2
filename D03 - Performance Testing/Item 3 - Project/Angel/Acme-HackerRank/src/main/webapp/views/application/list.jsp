...
<display:table name="applications" id="row" requestURI="${requestURI}"
	...
	<display:column titleKey="application.show">
		<security:authorize access="hasRole('HACKER')">
			<a href="application/hacker/show.do?idApplication=${row.id}"><spring:message
	...
	<display:column titleKey="application.problem">
		<security:authorize access="hasRole('HACKER')">
			<a href="problem/hacker/show.do?id=${row.problem.id}"> <jstl:out
	...
	<security:authorize access="hasRole('HACKER')">
		<display:column titleKey="application.createAnswer">
			<jstl:if test="${row.answer == null}">
				<a href="answer/hacker/create.do?idApplication=${row.id}"><spring:message
						code="application.create.answer" /> </a>
			</jstl:if>
			<jstl:if test="${!(row.answer == null)}">
				<a href="answer/hacker/show.do?idApplication=${row.id}"><spring:message

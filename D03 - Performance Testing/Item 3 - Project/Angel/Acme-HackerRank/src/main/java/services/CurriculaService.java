...
public class CurriculaService extends AbstractService {
	...
	public Collection<Curricula> findAllByHacker() {
		Hacker h;
		h = (Hacker) this.repository.findActorByUserAccountId(LoginService.getPrincipal().getId());

		try {
			return this.repository.findAllByHacker(h.getId());
	...
	public Curricula save(final Curricula save) {
		Assert.isTrue(super.findAuthority(LoginService.getPrincipal().getAuthorities(), Authority.HACKER));
		...
		if (save.getId() == 0)
			...
			Assert.isTrue(save.getHacker().equals(h));

...
<%--	<form:label path="sender">
	...
	: ${form.sender.name} ${form.sender.surname} (${form.sender.email})
	<br>
	<br> --%>
	<acme:textbox code="message.sender" path="sender" readonly="true"/>
	<acme:textbox code="message.subject" path="subject" readonly="${view}"/>
	<acme:textbox code="message.momentsent" path="momentsent" readonly="true"/>
...
<%-- 			<acme:select items="${priorities}" code="message.priority" path="priority"/> --%>
 		</jstl:if>  
	...
	<acme:textbox code="message.body" path="body" readonly="${view}"/>
	
	<jstl:if test="${not view}">
	<acme:submit name="send" code="message.save" />
	</jstl:if>
	<jstl:if test="${view}">
	<a href="message/dbox.do?boxId=${trash}&mess=${form.id}"><spring:message code="message.move" /></a>
	</jstl:if>
	<jstl:if test="${view}">
		<br>
		...
		<div
			...
			<jstl:forEach items="${boxesOptional}" var="var">
				<spring:message code="message.boxes" />: 
				<a href="message/dbox.do?boxId=${var.id}&mess=${form.id}">
					${var.name} </a>
				<br>
			</jstl:forEach>
			<br>

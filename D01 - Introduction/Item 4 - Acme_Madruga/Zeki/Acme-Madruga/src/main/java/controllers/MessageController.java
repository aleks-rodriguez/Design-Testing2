...
public class MessageController extends AbstractController {
	...
	public ModelAndView sendMessage(@ModelAttribute("`message`") final MessageForm form, final BindingResult binding) {
		ModelAndView result;

		System.out.println("sender: " + form.getSender());
		System.out.println("received " + form.getReceiver().toString());
		if (binding.hasErrors()) {
			result = this.createEditModelAndView2(form);
			result.addObject("errors", binding.getAllErrors());
		} else
			try {
				//				form = this.messageService.reconstruct(form, binding);
				...
				result = this.createEditModelAndView2(form, "message.commit.error");
	...
	protected ModelAndView createEditModelAndView(final Message mess, final String message) {
		...
		result.addObject("priorities", Utiles.priorities());
		result.addObject("sender", mess.getSender());

		return result;
	}

	protected ModelAndView createEditModelAndView2(final MessageForm message) {
		ModelAndView result;

		result = this.createEditModelAndView2(message, null);

		return result;
	}
	protected ModelAndView createEditModelAndView2(final MessageForm mess, final String message) {
		ModelAndView result;

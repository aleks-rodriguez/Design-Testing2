...
public class PositionController extends AbstractController {
	...
	public ModelAndView delete(final Position position) {
		...
		try {
			this.servicePosition.delete(position.getId());
			result = new ModelAndView("redirect:list.do");
		} catch (final Throwable opps) {
			result = this.createEditModelAndView(position, "profile.commit.error");
		}
		return result;
	}
	...
	protected ModelAndView createEditModelAndView(final Position position, final String message) {
		ModelAndView result;
		result = this.editFormsUrlId(position, "position/administrator/edit.do", "id", "/position/administrator/list.do", this.custom(new ModelAndView("position/edit")));
